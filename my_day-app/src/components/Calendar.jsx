...
export default function Calendar() {
    ...
    const getNumberOfDaysInMonth = (year, month) => 32 - new Date(year, month, 32).getDate();
    ...
    for (let j = 0; j < 7; j++) { // the first row can start at any position
        daysOfSelectedMonth[0][j] = (j >= positionToStart) ? dayIncrement++ : null;
    }
    for (let i = 1; i < 6; i++) {
        for (let j = 0; j < 7; j++) {
            daysOfSelectedMonth[i][j] = (dayIncrement <= maxNumberOfDays) ? dayIncrement++ : null;
        }
    }

    useEffect(() => {
        const API_PATH = 'http://localhost:3300/calendar?dateMin=2020-03-01&dateMax=2020-03-31';
